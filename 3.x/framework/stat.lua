...
function Stat:Analyst(total_time)
	...
	for name, process_data in pairs(data) do
		print(string.format("[%s] %6.4f%% (%7.4f)", name, process_data.run_time * 100 / total_time, process_data.run_time))
	...
	for index = 1, 10 do
		local data = tb[index]
		if data then
			print(string.format("[%d] %5.3f%%(%7.4f) %s count:%d", index, data.run_time * 100 / total_time, data.run_time, data.name, self.total_func_time[data.name].count))
		end
	end
	print("======Top Event=========")
	local tb = {}
	...
	for index = 1, 10 do
		local data = tb[index]
		if data then
			print(string.format("[%d] %s count:%d", index, data[1], data[2]))
		end
	end
end
...
local function TestStat()
	...
	Stat:SetRunning(0)
	local time = socket:gettime() - stat_time
	Stat:Analyst(time)
	return 1
end
