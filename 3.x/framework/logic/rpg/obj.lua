...
function RpgObj:GetNextCommand()
    ...
    return cmd_node:GetNextCommand()
end
...
function RpgObj:DodgeDamage(luancher_id)
    local event_name = self:GetClassName()..".HIT_MISS"
...
function RpgObj:Dead(dead_type)
    ...
    local luancher_id = self:GetLastDamager()
    Event:FireEvent(event_name, self:GetId(), luancher_id, dead_type)
end
